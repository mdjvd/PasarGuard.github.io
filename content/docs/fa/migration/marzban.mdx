...
<Tabs items={['SQLite', 'MySQL']}>
  ...
  <Tab value="MySQL">
    <div dir="rtl">
      ...
      <Callout type="info"> 
      💬 **نکته**:
      اگه در حال حاضر در آدرس دیتابیس از کاربر `marzban` به جای `root` استفاده می‌کنید و متغیرهای `MYSQL_DATABASE` و `MYSQL_USER` رو در فایل `.env` دارید، می‌تونید مقدار `marzban` اونها رو به `pasarguard` تغییر بدید. یا اینکه مثل مثال بالا از کاربر `root` استفاده کنید.
      </Callout>
    </div>
  </Tab>
</Tabs>

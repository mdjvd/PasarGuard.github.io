...
<Tabs items={['SQLite', 'MySQL']}>
  ...
  <Tab value="MySQL">
    <div dir="rtl">
      ...
      > ⚠️ **توجه**: در این مثال، قسمت `MYSQL_ROOT_PASSWORD` باید با مقدارش در فایل `.env` شما جایگزین بشه.
      ...
      <Callout type="info"> 
      💬 **نکته**:
      اگه در حال حاضر در آدرس دیتابیس از کاربر marzban به جای root استفاده می‌کنید و متغیرهای `MYSQL_DATABASE` و `MYSQL_USER` رو در فایل `.env` دارید، می‌تونید مقدار marzban اونها رو به pasarguard تغییر بدید. یا اینکه مانند مثال بالا از کاربر root استفاده کنید.
      </Callout>
    </div>
  </Tab>
</Tabs>

...
<Tabs items={['SQLite', 'MySQL']}>
  ...
  <Tab value="MySQL">
    ...
    <Callout type="info"> 
      💬 **Note**:
      If you are currently using `marzban` user instead of `root` in the database url and you have `MYSQL_DATABASE` and `MYSQL_USER` variables in `.env` file, you can change their `marzban` value to `pasarguard`. Or you can just use the `root` user like the example above.
    </Callout>

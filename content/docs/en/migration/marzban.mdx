...
<Tabs items={['SQLite', 'MySQL']}>
  ...
  <Tab value="MySQL">
    ...
    <Callout type="info"> 
💬 **Note**:
      If you used `marzban` user in database url before and you have `MYSQL_DATABASE` and `MYSQL_USER` variables in `.env' file, you can change the `marzban` value to `pasarguard`. Or use the `root` user like the example above.
    </Callout>
  </Tab>
</Tabs>
